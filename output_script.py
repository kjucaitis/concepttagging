...
# For every column which includes a flavor replace name with that flavor and save to tags list
...
for node in G.nodes():
    labels[node] = node.split('_mapped_clean_')[-1]  # This assigns the node's name as the label
...
# Print or process the connected components
...
# Define the keywords you want to search for
flavors_keywords = metrics + ['concept_id']
print(flavors_keywords)
# Filter columns that contain any of the keywords
#tag_filter = [col for col in slice.columns if any(keyword in col for keyword in keywords)]
...
# Rename columns dynamically
flavors_slice.columns = tags_slice.columns.str.replace('ingredients mentioned in the flavor names_mapped_clean_', '')
...
print(ts_correlation_matrix.stack())
...
ts.to_csv('Data/binary_tag_slice.csv')
...
with open('output_script.py', 'w', encoding='utf-8') as script_file:
